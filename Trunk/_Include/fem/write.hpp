...
namespace fem {
    ...
    // TODO copy-constructor potential performance problem
  {
    ...
    public:
      ...
      write_loop&
        operator,(
        str<StrLen> const& val) 
      {
        return operator,(str_cref(val.elems, val.len()));
      }
      write_loop&
        operator,(
        std::string const& val) 
      {
        return operator,(str_cref(val.data(), val.size()));
      }
      write_loop&
      operator,(
        ...
        arr_cref<T, Ndims> const& val)
      {
        ...
        for(size_t i=0;i<n;i++) {
          (*this), (T)val_begin[i]; // write_loop::operator,((T)val_begin[i]);    
        }
        return *this;
      }
